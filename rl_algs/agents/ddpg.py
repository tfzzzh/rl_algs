...
class DDPG(nn.Module):
    def __init__(
        ...
        self.use_reparameter_entropy = True
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor, next_qs_target: torch.Tensor) -> torch.Tensor:
        """
        ...
        assert next_qs.shape == next_qs_target.shape

        # TODO(student): Implement the different backup strategies.
        if self.target_critic_backup_type == "doubleq":
            next_qs = next_qs_target

        elif self.target_critic_backup_type == "min":
            next_qs = torch.minimum(next_qs, next_qs_target)
            next_qs = torch.min(next_qs, dim=0).values

        elif self.target_critic_backup_type == "min_target":
            next_qs = torch.min(next_qs_target, dim=0).values

        elif self.target_critic_backup_type == "mean":
            next_qs = (next_qs + next_qs_target) / 2.0
            next_qs = torch.mean(next_qs, dim=0)

        else:
            # Default, we don't need to do anything.
            assert self.target_critic_backup_type == "redq"
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action_distribution: torch.distributions.Distribution = self.actor(next_obs) # [bsize, action_dim]
            next_action = next_action_distribution.sample() #[bsize, action_dim]

            # Compute the next Q-values for the sampled actions
            next_qs = self.critic(next_obs, next_action) #[num_critic, bsize]
            next_qs_target = self.target_critic(next_obs, next_action)

            # Handle Q-values from multiple different target critic networks (if necessary)
            # (For double-Q, clip-Q, etc.)
            next_qs = self.q_backup_strategy(next_qs, next_qs_target)
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                # next_action_entropy = 0.0 # TODO: delete it
                next_qs += self.temperature * next_action_entropy

            # Compute the target Q-value
            done_reshape = done.int().unsqueeze(dim=0) # [1, bsize]
            target_values: torch.Tensor = reward + self.discount * (1 - done_reshape) * next_qs
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # I compute q value use critic (not target critic)
            mult = [1 for _ in range(obs.ndim + 1)]
            mult[0] = self.num_actor_samples
            q_values = self.critic(obs[None].repeat(mult), action)
            ...
            q_values = torch.mean(q_values, axis=0) # [samples, bsize]
            advantage = q_values

        # Do REINFORCE: calculate log-probs and use the Q-values
        # TODO(student)
        log_probs = action_distribution.log_prob(action) # [sample, bsize]
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        # # TODO: just for test remove it (two line)
        # entropy = self.entropy(self.actor(obs)).mean()
        # loss = -entropy # max entropy -> min loss
    ...
    def update(
        ...
        if self.soft_target_update_rate is not None:
            assert self.soft_target_update_rate >= 0.0 and self.soft_target_update_rate <= 1.0
